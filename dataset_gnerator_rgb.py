...
output_base_path = "working_dataset_rgb/"

for class_name in classes:
    class_path = "dataset/test/" + class_name
    for video in os.listdir(class_path):
        input_video_path = class_path + video
        temporal_gradient_output_path = (
            os.path.join(output_base_path, "test", class_name, video.split(".")[0])
            + ".mp4"
        ...
        temporal_gradient_out = cv2.VideoWriter(
            temporal_gradient_output_path, fourcc, 20.0, (224, 224)
        )
        ...
        while True:
            ...
            if movement_frame.shape[0] != 0 and movement_frame.shape[1] != 0:
                ...
                print(movement_frame.shape)
                temporal_gradient_out.write(movement_frame)

                cv2.imshow("res", movement_frame)
            k = cv2.waitKey(1)
            if k == 27:
                break
